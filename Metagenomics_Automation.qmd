...
You can do this by going into the "classification" folder within the appropriate barcode and opening up the **report_standard.txt** file. There you can set the number of reads back to zero for the false positive finding - remember to also re-set the normalized percentage. Save the file with the same name.

::: callout-warning
## Warning

For the next steps, remember to be in the correct folder and update the paths. 
:::

``` bash
cd /mnt/viro0002-data/sequencedata/processed/Diagnostics_metagenomics/Metagenomics_automation/Viro_Run_001/barcode01/krona/
```
...
``` bash
chmod 777 -R /mnt/viro0002-data/sequencedata/processed/Diagnostics_metagenomics/Viro_Run_001/
```
